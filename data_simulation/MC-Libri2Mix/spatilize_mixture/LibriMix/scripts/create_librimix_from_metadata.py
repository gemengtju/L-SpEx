...
parser.add_argument('--params_dir', type=str, required=True,
                    help='Path to the LibriMix RIR params directory')
...
def main(args):
    ...
    create_librimix(librispeech_dir, wham_dir, librimix_outdir, metadata_dir, params_dir,
                    freqs, n_src, modes, types)


def create_librimix(librispeech_dir, wham_dir, out_dir, metadata_dir, params_dir,
                    freqs, n_src, modes, types):
    ...
    for md_filename in md_filename_list:
        ...
        params_csv_path = os.path.join(params_dir, md_filename)
        process_metadata_file(csv_path, params_csv_path, freqs, n_src, librispeech_dir,
                              wham_dir, out_dir, modes, types)


def process_metadata_file(csv_path, params_csv_path, freqs, n_src, librispeech_dir, wham_dir,
                          out_dir, modes, types):
    """ Process a metadata generation file to create sources and mixtures"""
    md_file = pd.read_csv(csv_path, engine='python')
    params_file = pd.read_csv(params_csv_path, engine='python')
    for freq in freqs:
        ...
        for mode in modes:
            ...
            if types == ['mix_clean']:
                ...
                if 'mix_reverb' in types:
                    subdirs = subdirs + [f's{i + 1}_reverb' for i in range(n_src)]
                if 'mix_anechoic' in types:
                    subdirs = subdirs + [f's{i + 1}_anechoic' for i in range(n_src)]
            # Create directories accordingly
            for subdir in subdirs:
                os.makedirs(os.path.join(dir_path, subdir))
            # Go through the metadata file
            process_utterance(md_file, params_file, librispeech_dir, wham_dir, freq, mode,
                              subdirs, dir_path, subset_metadata_path, n_src)


def process_utterance(md_file, params_file, librispeech_dir, wham_dir, freq, mode, subdirs,
    ...
    for index, row in tqdm(md_file.iterrows(), total=len(md_file)):
        ...
        # TODO: print(transformed_sources)
        for subdir in subdirs:
            if subdir == 'mix_clean':
            ...
            elif subdir == 'mix_reverb':
                params_row = params_file[params_file['mixture_ID'] == mix_id]
                sources_to_mix = generate_reverb([params_row['room_x'].iloc[0], params_row['room_y'].iloc[0], params_row['room_z'].iloc[0]],
                                [[params_row['mic1_x'].iloc[0], params_row['mic_y'].iloc[0], params_row['mic_z'].iloc[0]],
                                 [params_row['mic2_x'].iloc[0], params_row['mic_y'].iloc[0], params_row['mic_z'].iloc[0]],
                                 [params_row['mic3_x'].iloc[0], params_row['mic_y'].iloc[0], params_row['mic_z'].iloc[0]],
                                 [params_row['mic4_x'].iloc[0], params_row['mic_y'].iloc[0], params_row['mic_z'].iloc[0]]],
                                 [[params_row['s1_x'].iloc[0], params_row['s1_y'].iloc[0], params_row['s1_z'].iloc[0]],
                                 [params_row['s2_x'].iloc[0], params_row['s2_y'].iloc[0], params_row['s2_z'].iloc[0]],
                                 [params_row['s3_x'].iloc[0], params_row['s3_y'].iloc[0], params_row['s3_z'].iloc[0]]],
                                 params_row['T60'].iloc[0], n_src=n_src, fs=freq, signals=transformed_sources[:n_src], subdir=subdir)
                write_reverb_sources(mix_id, sources_to_mix, subdir, dir_path, freq, n_src)
            elif subdir == 'mix_anechoic':
                params_row = params_file[params_file['mixture_ID'] == mix_id]
                sources_to_mix = generate_reverb([params_row['room_x'].iloc[0], params_row['room_y'].iloc[0], params_row['room_z'].iloc[0]],
                                [[params_row['mic1_x'].iloc[0], params_row['mic_y'].iloc[0], params_row['mic_z'].iloc[0]],
                                 [params_row['mic2_x'].iloc[0], params_row['mic_y'].iloc[0], params_row['mic_z'].iloc[0]],
                                 ...
                                 params_row['T60'].iloc[0], n_src=n_src, fs=freq, signals=transformed_sources[:n_src], subdir=subdir)
                write_reverb_sources(mix_id, sources_to_mix, subdir, dir_path, freq, n_src)
                # TODO: write_single_reverb / original speech is s1_direct / s2_direct
            else:
                continue

            # TODO: elif subdir == 'mix_reverb':
            #           sources_to_mix = synthesis_reverb(transformed_sources[:n_src], rir)
...
def generate_reverb(room_dim, mics_pos, sources_pos, T60, n_src, fs, signals, subdir):
    # We invert Sabine's formula to obtain the parameters for the ISM simulator
    e_absorption, max_order = pra.inverse_sabine(T60, room_dim)
    if subdir == "mix_anechoic":
        max_order = 0
    room = pra.ShoeBox(room_dim, fs=8000, materials=pra.Material(e_absorption), max_order=max_order)

    # add sources and mic
    for i in range(n_src):
        room.add_source(sources_pos[i], signal=signals[i])
    room.add_microphone_array(pra.MicrophoneArray(np.array(mics_pos).T, fs))
    room.compute_rir()
    audio_array = room.simulate(return_premix=True) # (n_src, n_mic, len)
    audio_array = np.transpose(audio_array, (0, 2, 1))
    new_sources_list = []
    speech_length = len(signals[0])
    for i in range(audio_array.shape[0]):
        new_sources_list.append(audio_array[i,:speech_length,:])
    #print("reverb_shape:", room.mic_array.signals.shape)
    #print("reverb:", room.mic_array.signals)
    #plt.plot(room.mic_array.signals[1,:])
    #plt.savefig('./reverb.jpg')

    return new_sources_list



#def synthesis_reverb(sources_list, rir_path):
#    rir = sio.loadmat(rir_path)
#    new_sources_list = []
#    speech_length = len(sources_list[0])
#    for source_idx in range(len(sources_list)):
#        h_reverb = rir['h_reverb_'+str(source_idx+1)]
#        channel_num = len(h_reverb)
#        reverbsyn = np.zeros((speech_length, channel_num)) 
#        for rir_idx in range(channel_num):
#            # the conv operation is borrowed from pyroomacoustics
#            # (https://github.com/LCAV/pyroomacoustics/blob/218c0ec3e8422f1ede30de684520c782a500f9ff/pyroomacoustics/room.py)
#            # PS: we can also use this implement version 
#            # (https://github.com/sunits/Reverberated_WSJ_2MIX/blob/b3734060e90876672f486514815644084c6c5007/olafilt.py#L6)
#            lreverb = fftconvolve(h_reverb[rir_idx], sources_list[source_idx])
#            reverbsyn[:, rir_idx] = lreverb[:speech_length]
#        new_sources_list.append(reverbsyn)
#    return new_sources_list
...
def create_empty_metrics_md(n_src, subdir):
    ...
    if subdir == 'mix_clean':
        ...
        metrics_dataframe[f"noise_SNR"] = {}
    elif subdir == 'mix_reverb' or subdir == 'mix_anechoic':
        for i in range(n_src):
            metrics_dataframe[f"source_{i + 1}_SNR"] = {}
...
def write_reverb_sources(mix_id, reverb_sources, subdir, dir_path, freq, n_src):
    # Write reverb and anechoic sources
    ex_filename = mix_id + '.wav'
    for i in range(n_src):
        save_path = os.path.join(dir_path, "s"+str(i+1)+"_"+subdir.split("_")[-1], ex_filename)
        abs_save_path = os.path.abspath(save_path)
        sf.write(abs_save_path, reverb_sources[i], freq)
